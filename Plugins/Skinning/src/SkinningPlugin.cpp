...
QWidget* SkinningPluginC::getWidget() {
    ...
    connect( m_widget, &SkinningWidget::showWeightsType, this, &SkinningPluginC::onShowWeightsType );
    return m_widget;
}
...
void SkinningPluginC::onCurrentChanged( const QModelIndex& current, const QModelIndex& prev ) {
    ...
    if ( it.m_entity )
    {
        ...
        if ( comps.size() != 0 )
        {
            ...
            if ( it.isRoNode() )
            {
                auto animComp = dynamic_cast<AnimationPlugin::AnimationComponent*>( it.m_component );
                if ( animComp )
                {
                    // get the bone's limb
                    comp->setWeightBone( animComp->getBoneIdx( it.m_roIndex ) );
                }
            }
        } else
...
SkinningWidget::SkinningWidget( QWidget* parent ) : QFrame( parent ), m_current( nullptr ) {
    ...
    connect( m_actionSTBSLBS, &QAction::triggered, this, &SkinningWidget::onSTBDLBSActionTriggered );
    connect( m_actionSTBSDQS, &QAction::triggered, this, &SkinningWidget::onSTBSDQSActionTriggered );
}
...
void SkinningWidget::onSTBDLBSActionTriggered() {
